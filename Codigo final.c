...
//Aqui declaramos as variaveis necessarias.
    int sessao,i,numeroPessoa[2],j,masculino=0,feminino=0,idade[100],ingresso[100],kid=0,adolescente=0,adult=0,idoso=0;
    int masculino18=0,femenino18=0,inteiratotal1=0,meiatotal1=0,inteiratotal2=0,meiatotal2=0,contador1=0,contador2=0,contador3=0,contador4=0;
    ...
    for(i=0;i<sessao;i++) // usamos o for para repetir as sessões.
    {
        ...
        scanf("%i",&numeroPessoa[i]);
        while(numeroPessoa[i]<=10){   
            printf("Por favor selecione um numero maior que 10: ");
            scanf("%i",&numeroPessoa[i]);
        }
        
        for(j=0;j<numeroPessoa[i];j++){  // Aqui usamos o for para cadastrar as pessoas que estão na sessão.
            ...
            if (genero[j] == 'm' && idade[j]>=18){ 
                masculino18=masculino18+1;
            }
            else if (genero[j] == 'f' && idade[j]>=18){
                femenino18=femenino18+1;
            }
    ...
    if(contador1+contador2>contador3+contador4){
        printf("\nHouveram mais pagamentos de inteiras");
    }
    else if(contador1+contador2<contador3+contador4){
